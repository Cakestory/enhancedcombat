...
public final class Helpers extends SynchedSettings {
	...
	public static float getOffhandDamage(EntityPlayer player) {
		Multimap<String, AttributeModifier> modifiers = player.getHeldItemOffhand().getAttributeModifiers(EntityEquipmentSlot.MAINHAND);
		float attackModified = 0.0F;
		for (Map.Entry<String, AttributeModifier> modifier : modifiers.entries()) {
			if (modifier.getKey().contains("attackDamage")) {
				attackModified = (float) modifier.getValue().getAmount();
			}
		}
			...
			if (!targetEntity.hitByEntity(player)) {
				...
				if (damage > 0.0F || cMod > 0.0F) {
					...
					if (offhand) {
						...
						attacked = execNullable(targetEntity.getCapability(EventHandlers.SECONDHURTTIMER_CAP, null), sht -> sht.attackEntityFromOffhand(targetEntity, DamageSource.causePlayerDamage(player), attackDmgFinal), false);
					} else {
						...
						if (isSword) {
							for (EntityLivingBase living : player.world.getEntitiesWithinAABB(EntityLivingBase.class, targetEntity.getEntityBoundingBox().grow(1.0D, 0.25D, 1.0D))) {
								if (living != player && living != targetEntity && !player.isOnSameTeam(living) && player.getDistanceSq(living) < 9.0D) {
									...
									if (offhand) {
										execNullable(targetEntity.getCapability(EventHandlers.SECONDHURTTIMER_CAP, null), sht -> sht.attackEntityFromOffhand(living, DamageSource.causePlayerDamage(player), 1.0F));
									} else {
	...
	/**
	 * This returns a null value for those final variables that have their values
	 * injected during runtime. Prevents IDEs from warning the user of potential
	 * NullPointerExceptions on code using those variables.
	 * 
	 * @param <T> any type
	 * @return null
	 */
